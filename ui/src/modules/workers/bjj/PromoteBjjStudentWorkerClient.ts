...
export default class PromoteBjjStudentWorkerClient implements PromoteZkClient {
...
{
  await this._call('promoteStudent', { studentPublicKey, rank, instructorPublicKey });
}
async updateBackingStore(studentPublicKey: string, rank: string, instructorPublicKey: string): Promise<void>
{
  await this._call('updateBackingStore', { studentPublicKey, rank, instructorPublicKey });
}
  ...
  constructor() {
    this.worker = new Worker(new URL('./PromoteBjjStudentWorker.ts', import.meta.url))
    this.promises = {};
    this.nextId = 0;
