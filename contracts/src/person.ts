import { Field, Mina, PrivateKey, MerkleMap, PublicKey } from 'snarkyjs';

import { Sender, HasMerkleMapContract } from './index.js';
...
class Personas {
  ...
  constructor(instance: HasMerkleMapContract, sender: Sender) {
   ...
   */
  async addNew(uid: string, data: any) {
    // set up a change to the MerkleMap
    const key = Field(uid);
    const value = Field(data as number);
    this.merkleMap.set(key, value);
    console.log(
      `\nmodels.Persona: merkleMap key=${key.toString()}, value=${this.merkleMap.get(
        key
      )}`
    );
    ...
    const txn1 = await Mina.transaction(this.sender.publicKey, () => {
      this.contract.addPersona(newRoot);
    });
  ...
  async update(uid: string, data: any) {
    ...
    const txn1 = await Mina.transaction(this.sender.publicKey, () => {
      this.contract.updatePersona(witness, key, newValue, currentValue);
    });
    console.log(
      `\nmodels.Persona: contract.updatePersona txn1=`,
      txn1.toPretty()
    );
